...
function jumpPage(myform){//页面go跳转
	
	var pageCount= document.getElementById("pageCount");
	var p= document.getElementById("jumppage");
	if(/^[1-9]\d*$/.test(p.value)){
		if(p.value<=pageCount.value)
			{
			/* alert("输入的页数为："+p.value); */
		   document.myform.action="/classroomweb/roomServlet?action=list&&page="+p.value;
       	   document.myform.submit(); 
...
function batch_del(){//批量删除
	...
	var checkbox = document.getElementsByName("my_checkbox");
	var j = 0;
	...
	if (j == 0) {
		...
		document.myform.action = "/classroomweb/roomServlet?action=deleteAll";
		document.myform.submit();
	}
 ...
 <hr>
  	<table id="room_table" width="100%" border="0" cellSpacing=0 cellPadding=3>
  	 ...
  	 <tr height=35 bgcolor="#FFFFFF">
  	 	<td colspan="9" align="center">很抱歉，教室表中还没有数据</td>
  	 </tr>
  	 <%}else{ 
  	 ...
  	 %>
  	 <tr height=35 bgcolor="#FFFFFF" onMouseOver="this.bgColor='#CDE6FF'" onMouseOut="this.bgColor='#FFFFFF'">
